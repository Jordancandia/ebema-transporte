...
export function renderTariffTransportView(container) {
  ...
  container.innerHTML = `
    ...
    <div class="flex gap-sm mb-lg border-b border-outline-variant pb-sm overflow-x-auto" id="tt-subtabs">
      ...
      ${subTabButton('zapsap', 'table', 'ZCAP/SAP')}
...
async function calcularPeajes(content, db, cfg, rutas, { force = false } = {}) {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    let distanceKm     = null;
    let googleHasTolls = null; // null = Google falló (no concluyente)

    try {
      const g = await callGoogleTollCheck(originCity, destCity);
      distanceKm     = g.distanceKm ?? null;
      googleHasTolls = g.hasTolls   ?? null;
    } catch (err) {
      console.warn('Google distance falló para', ruta.codigo, err.message);
    }

    tollTargetsPrep.push({ ruta, originCity, destCity, distanceKm, googleHasTolls });
    await sleep(150);
  }
  ...
  for (let i = 0; i < tollTargets.length; i++) {
    if (cancelado) break;
    const { ruta, originCity, destCity, googleHasTolls } = tollTargets[i];
    ...
    for (const ejes of [2, 3]) {
      ...
      try {
        ...
        // Si GetAPI no encontró la ruta pero Google confirmó que NO hay peajes → $0 automático
        // Si googleHasTolls===null (Google falló) → dejar notFound para REVISIÓN (conservador)
        if (ida && ida.notFound && googleHasTolls === false) {
          ida = { tollCLP: 0, mainlineCLP: 0, rampCLP: 0, electronicCLP: 0,
                  hasToll: false, notFound: false, source: 'google-confirmed-zero' };
        }
        ...
        if (!ida || ida.notFound) {
          ...
          await sleep(500);
          if (vuelta && vuelta.notFound && googleHasTolls === false) {
            vuelta = { tollCLP: 0, mainlineCLP: 0, rampCLP: 0, electronicCLP: 0,
                       hasToll: false, notFound: false, source: 'google-confirmed-zero' };
          }
        }
      } catch (err) {
