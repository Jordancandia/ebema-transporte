import { getDatabase, saveDatabase, getCentreName, getOrigenGroups, calcEjes, deleteRow, deleteRows } from './data.js?v=202609221735';
...
export function renderFichaTransporte(container, transportId) {
  ...
  document.getElementById('btn-back-transports').addEventListener('click', () => {
    ...
    if (sesion && sesion.tipo === 'proveedor') {
      ...
      import('./provider-portal.js?v=202609221735').then(m => m.renderPortalHome(stage));
    } else {
      if (title) title.textContent = 'Gestión de Transportes';
      import('./transports.js?v=202609221735').then(m => m.renderTransportsView(stage));
    }
  });
