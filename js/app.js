import { getDatabase, saveDatabase, initDatabase, loadRoutesData } from './data.js?v=202609181211';
...
function prewarmMods() {
  setTimeout(() => loadMod('ind',   './indicadores.js?v=202609181211'), 600);
  setTimeout(() => loadRoutesData(), 800);  // pre-fetch tablas pesadas en background
  setTimeout(() => loadMod('abast', './abastecimiento.js?v=202609181211'), 2000);
}
...
function renderApp() {
  if (!currentSession) {
    ...
    import('./provider-portal.js?v=202609181211').then(m => m.renderProviderShell(currentSession, handleLogout));
  } else {
...
async function switchTab(tabName, subName = null) {
  ...
  switch (tabName) {
    case 'home': {
      ...
      const m = await loadMod('ind', './indicadores.js?v=202609181211');
    ...
    case 'transports': {
      ...
      const m = await loadMod('trans', './transports.js?v=202609181211');
    ...
    case 'routes': {
      ...
      const m = await loadMod('routes', './routes.js?v=202609181211');
    ...
    case 'roles': {
      ...
      const m = await loadMod('roles', './roles.js?v=202609181211');
    ...
    case 'tarifas-transporte': {
      ...
      const m = await loadMod('tt', './tarifas-transporte.js?v=202609181211');
    ...
    case 'tarifas-clientes': {
      ...
      const m = await loadMod('tc', './tarifas-clientes.js?v=202609181211');
    ...
    case 'abastecimiento': {
      ...
      const m = await loadMod('abast', './abastecimiento.js?v=202609181211');
    ...
    case 'flete-tercero': {
      ...
      const m = await loadMod('fter', './flete-tercero.js?v=202609181211');
